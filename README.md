...
3. Clockifyのユーザー設定で取得したAPIキーを入力します。「接続してWorkspaceを取得」を押し、対象Workspaceを選択します。
4. 固定時給を確認し「保存して開始」を押します。APIキーをチャットに送る必要はありません。
...
- BREAKは金額計算の対象外です。今日の累計・ログイン時自動起動は未実装です。
...
今後の候補: 実アカウントでの同期とKeychainの検証、今日の累計金額、ログイン時の自動起動。
